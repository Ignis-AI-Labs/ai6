---
description: Enter ai6 paired-review mode — Claude (Opus) reviews each completed work unit
---
...
1. **Dispatch a review.** Call the **`ai6_review`** tool with:
   ...
   Claude Reviewer and returns its review. (If the `ai6_review` tool is unavailable,
   fall back to: `bash ~/.ai6/ask-claude.sh "<context>" <files...>`.)
