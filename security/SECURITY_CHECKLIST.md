...
**Version:** 1.1 — June 2026 *(see Appendix C: Version History for changes from 1.0)*  
...
> **v1.1 adds a self-run scanning layer** (§3.5, §4.7, §5.7, §10.4) — the tools every engineer and the founder run *themselves*, continuously, before anything reaches a CSO or an external audit. The full audit protocol above this layer is unchanged. The operating standard is now four nets, not one: self-run scans catch the obvious, CI gates the merge, the CSO reviews depth, and external firms validate before mainnet.
...
- [ ] **[DEPLOYMENT BLOCKER]** For any live AMM/DLMM/DAMM: pool-math invariants are fuzzed and passing *before liquidity scales* — at minimum, the floor cannot be pushed below the defended bid wall, and the curve cannot be drained, under any sequence of swaps, deposits, and withdrawals. No invariant suite, no scale.
...
### 3.5 Recommended Application Security Tooling (2026) [SELF-RUN LAYER]

*The 2025–26 shift: from pattern-matching SAST to **semantic** SAST that understands data flow and catches business-logic flaws (broken auth, IDOR, authorization bypass) that rule-based scanners structurally miss. AI-assisted coding ships more code faster, and logic bugs are exactly what it introduces — so this layer is now mandatory, not optional.*

**SAST (source code):**
- [ ] **Semgrep** runs as the baseline SAST in CI and locally — diff-aware, custom rules tuned to project authz patterns. Free tier is sufficient to start.
- [ ] An **AI-native SAST** (ZeroPath, DryRun Security, or Aikido) piloted against the primary repos for business-logic depth Semgrep misses — authorization bypass and multi-step logic flaws specifically.
- [ ] **CodeQL** (GitHub code scanning) enabled as a free second opinion — not relied on as the primary.

**DAST (running app + APIs):**
- [ ] **OWASP ZAP** + **Nuclei** run against staging on a schedule — runtime exploits and known-CVE/misconfiguration coverage. Both free, both self-run.
- [ ] **Burp Suite** (Pro for serious work) used for hands-on manual probing of auth flows and business logic.
- [ ] DAST scanner verified able to authenticate and drive session/login flows — an unauthenticated scan never reaches the protected routes where the valuable logic lives.

**Standard:** SAST and DAST are complementary and both required — SAST reads code at rest, DAST attacks it running. Neither replaces the other, and neither replaces manual review. Tool-found severity Critical/High blocks the PR; results feed the CSO review, they do not substitute for it.
...
- [ ] **[CRITICAL]** Full git-history scan (gitleaks) run across every repo — pre-commit hooks stop new leaks but do not find what is already buried in history
- [ ] Found secrets verified for live/active status (TruffleHog verification mode) and rotated immediately if active — a leaked Supabase service key or treasury signer in old history is a fatal-class mistake
- [ ] Platform-level push protection enabled (e.g., GitHub Secret Scanning + Push Protection) to block secrets at push time
...
### 4.7 Cloud Posture, IaC, and Database Isolation [HIGH] [SELF-RUN LAYER]

*Misconfiguration remains the most common real-world breach vector. Verify posture with a tool, not from memory or a dashboard toggle.*

- [ ] **Checkov** or **tfsec** scans all infrastructure-as-code (Terraform, etc.) for misconfiguration before deploy — runs in CI, free, self-run
- [ ] **Prowler** (or equivalent CSPM) run against any AWS/GCP/Azure footprint as it is added — hundreds of CIS-benchmark checks
- [ ] **[CRITICAL] Supabase Row Level Security proven, not assumed:** an automated test asserts RLS is enabled on every table *and* that policies actually deny cross-tenant/cross-user reads. The dashboard toggle is not evidence; a passing test is.
- [ ] Storage buckets and object stores verified non-public unless explicitly intended and documented
- [ ] Database connection strings and service-role keys never shipped to the client bundle — verified by build-output scan
...
### 5.7 Recommended AI Security Tooling (2026) [SELF-RUN LAYER]

*§5.1–5.6 define the standards and guardrails. This names the scanners that test them. Prompt injection is the #1 LLM threat two years running; for agentic systems, indirect injection and tool poisoning are the chained-exploit path. These are self-run and free — a clean report here puts you ahead of most teams shipping AI products.*

- [ ] **garak** (NVIDIA) run against every AI endpoint reachable by a user or any endpoint that reads untrusted data — the "nmap for LLMs": probes for prompt injection, jailbreaks, and data leakage
- [ ] **PyRIT** (Microsoft) used for automated adversarial red-teaming at scale against AI systems before release
- [ ] **promptfoo** integrated into CI as a prompt-injection / jailbreak-resistance eval harness — regressions block the release
- [ ] Results from the above fed into the §5.6 quarterly red-team metrics — longitudinal tracking, no silent degradation
...
- [ ] **Daily:** On-chain monitoring alerts reviewed; sanctions screening verified active; AI output monitoring reviewed; **field threat-intelligence reviewed — new CVEs, exploit disclosures, and advisories affecting our stack (Next.js, Rust crates, Supabase, Base, our dependencies, our model providers) triaged for "does this affect us?" with a logged yes/no/monitoring decision** (see §10.4)
...
### 10.4 Self-Run Scanning Cadence and Field Vulnerability Watch [SELF-RUN LAYER]

*This is the layer that runs without waiting for anyone. Every tool here is free or near-free, self-operated, and scoped strictly to assets we own or are authorized to test. Keep a one-line written scope note per run — this is the §AUDIT_PROTOCOL paper trail.*

**Weekly founder/engineer self-run scans (ranked by impact-per-minute):**
- [ ] `gitleaks` full-history scan across every repo — leaked keys
- [ ] `semgrep` on main app repos — baseline SAST
- [ ] Foundry/Echidna invariant run on pool math — the floor fuzz (highest DeFi value; see §1.9)
- [ ] `ZAP` + `nuclei` against staging — runtime + known-CVE
- [ ] `garak` against AI endpoints — prompt-injection probe (see §5.7)
- [ ] Supabase RLS assertion test — prove cross-tenant isolation (see §4.7)
- [ ] Output saved; anything flagged Critical/High logged as a finding in the issue tracker per §AUDIT_PROTOCOL

**Daily field vulnerability watch (the "is this us?" lens):**

The purpose is a daily lens on the field so a new vulnerability is assessed against *our* attack surface the day it surfaces — whether or not it affects us, we learn how it would and how we'd defend. Build this as a daily review backed by:
- [ ] Automated dependency advisories wired to alert: Dependabot/Renovate (npm, cargo, pip), GitHub Security Advisories, RustSec, Socket.dev real-time feed
- [ ] Curated intelligence sources monitored daily: CISA KEV (Known Exploited Vulnerabilities) catalog, OWASP updates, relevant vendor advisories (Vercel, Supabase, Base/OP Stack, model providers), and Web3-specific exploit disclosure feeds (rekt.news, audit-firm disclosures)
- [ ] Each material item triaged with a logged decision: **affects us / does not affect us / monitoring** — with a one-line rationale. "Does not affect us" still records *why*, so the reasoning is auditable later
- [ ] Items marked "affects us" routed immediately to the §10.2 re-audit trigger ("Critical CVE affecting a deployed dependency") and the §4.6 patch SLA
- [ ] A running log maintained — over time this becomes a record of how the team reasons about threats, and the training signal for hardening before we're hit

> **Design intent (per founder directive):** the daily check is not only an alerting mechanism — it is a learning discipline. The habit of asking "could this have hit us, and would we have survived it?" every day is what converts external incidents into our own preparedness. The goal is to take care of our own perimeter rather than wait to be saved.
...
> ⚠️ Trivy was compromised (March 2025, follow-on April 2026) — pin scanners and CI actions by SHA digest, never by mutable tag. Grype offers superior EPSS+CISA-KEV prioritization for open-source use.

**Secrets Management:** HashiCorp Vault, AWS Secrets Manager, GCP Secret Manager

**Application SAST (self-run layer):** Semgrep (baseline), ZeroPath / DryRun Security / Aikido (AI-native, business-logic depth), GitHub CodeQL (free second opinion)

**Application DAST (self-run layer):** OWASP ZAP, Nuclei, Burp Suite, StackHawk (dev-native CI)

**Secret Scanning (self-run layer):** gitleaks (full-history), TruffleHog (live verification), GitHub Secret Scanning + Push Protection

**Cloud Posture / IaC (self-run layer):** Checkov, tfsec, Prowler (CSPM)

**AI/LLM Security Scanners (self-run layer):** garak (NVIDIA), PyRIT (Microsoft), promptfoo

**Autonomous Pentest / Validation (platform):** Horizon3 NodeZero, Pentera, Hadrian Nova (external attack surface), Astra, XBOW, Picus. Open-source/sandboxed (own-assets only): PentAGI, Strix, Deadend CLI — powerful, rough-edged; isolated Docker, written scope, never against assets you don't control. Continuous validation *between* human pentests, never a replacement for qualified human sign-off (PCI/SOC 2 still require it).
...
**v1.1 — June 2026**
- Added §3.5 Recommended Application Security Tooling — AI-native SAST (Semgrep baseline; ZeroPath/DryRun/Aikido for business-logic depth) and self-run DAST (ZAP, Nuclei, Burp).
- Added §4.7 Cloud Posture, IaC, and Database Isolation — Checkov/tfsec/Prowler; made Supabase RLS a *proven-by-test* [CRITICAL] item rather than an assumed toggle.
- Added §5.7 Recommended AI Security Tooling — garak, PyRIT, promptfoo self-run scanners under the existing OWASP-LLM standards.
- Added §10.4 Self-Run Scanning Cadence and Field Vulnerability Watch — the weekly six-scan founder/engineer ritual and the daily "is this us?" threat-intelligence discipline.
- Strengthened §1.9 — invariant fuzzing of pool math is now an explicit [DEPLOYMENT BLOCKER] for any live AMM/DLMM/DAMM before liquidity scales.
- Strengthened §4.5 — full git-history secret scanning and live-verification added; pre-commit hooks alone are insufficient.
- Added daily field threat-intelligence line to §10.1 monitoring cadence.
- Expanded Appendix A with the self-run tooling categories and the Trivy supply-chain compromise caveat.
- Introduced the four-net operating model: self-run scans → CI gate → CSO depth review → external firm validation.
