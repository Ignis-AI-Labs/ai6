#!/usr/bin/env bash
#
# ai6 chunked review — guarantees the Reviewer sees every file IN FULL, even when a
# work unit is too large to fit a model's context window in one request.
#
# If the planned request exceeds AI6_MAX_CHARS, the files are split into multiple
...
ai6_review() {
  local budget="${AI6_MAX_CHARS:-200000}"
  ...
  [ "${overhead}" -gt "${budget}" ] && echo "ai6: WARNING — AGENTS.md + context (${overhead}B) alone exceeds AI6_MAX_CHARS (${budget}B); passes may still exceed the model window." >&2
  ...
  while [ "${i}" -lt "${#FILES[@]}" ]; do
    ...
    [ "${sz}" -gt "${avail}" ] && echo "ai6: WARNING — '${f}' (${sz}B) exceeds the per-pass budget (${avail}B); reviewing it alone (it may still exceed the model window)." >&2
  ...
  echo "ai6: review payload is large — splitting into ${n} passes so the reviewer sees every file in full." >&2
...
(ai6 chunked review: part ${k} of ${n}. Judge ONLY the ${#FILES[@]} file(s) in this part; the other parts are reviewed separately. Do not flag files as missing.)"
    ...
    verdicts+=( "${v}" )
    {
      echo "### ai6 review — part ${k} of ${n}  (verdict: ${v})"
      echo "Files: ${FILES[*]}"
      echo
      cat "${presp}"
      echo
  ...
  {
    echo "## ai6 Aggregate Verdict (${n} parts)"
    echo
